...
BmpRead::BmpRead(const char* FileName): FileIn(FileName) {
   ...
   { short id;
     // Kennung "BM"
     id = get_short();
     if ( id != 0x4d42 ) { // ASCII-Zeichen "BM" m?ssen hier stehen
       throw "BmpRead::BmpRead(char*): Error: Not a BMP-File\n";
     }
     // Dateigr??e
     FileSize = get_long();
     // Zwei reservierte Worte, m?ssen 0 enthalten
     if ((get_short()!=0)) {
#ifndef HOHE_BMP_FORMAT_TOLERANZ
       // Auskommentiert, da nicht alle Dateien diese Bedingung erf?llen.
   ...
   switch(ImageHeaderSize) {
     case 12:
       ...
       width    = get_short();
       height   = get_short();
       if (get_short()!=1) { // Anzahl der Planes mu? 1 sein!
   ...
   if (LUT_used) {
     ...
     // LUT ?berpr?fen, ob sie eine Grauwert-LUT ist
     GrayLUT = 1;
     for (i=0;i<ImageColorsUsed;i++) {
